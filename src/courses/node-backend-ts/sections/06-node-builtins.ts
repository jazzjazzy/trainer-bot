  ...
  interview: [
    {
      ...
      a: "PHP puts thousands of functions in the global namespace — file, string, array, JSON helpers are just there. Node keeps the core deliberately small and organised into modules you import explicitly: `node:http`, `node:fs`, `node:path`, `node:crypto` — and the ecosystem expects npm to provide higher-level tools like frameworks and ORMs. The `node:` prefix marks an import as a runtime builtin rather than an npm package, which prevents a malicious or typo-squatted `fs` package in node_modules from shadowing the real thing and makes intent obvious when reading imports. Modern code uses the prefix for every builtin.",
    },
    {
