  ...
  interview: [
    {
      ...
      a: "During rendering, `Renderer::doRender()` pushes a `BubbleableMetadata` frame per render-array level onto a `RenderContext` stack; when a subtree finishes, `RenderContext::bubble()` pops its frame and merges it into the parent's. Cache contexts and tags are unioned upward, and max-age is combined with `Cache::mergeMaxAges()`, which discards `Cache::PERMANENT` and returns the minimum. So a nested 'who's online' block declaring the `user` context makes the entire page vary per user: Dynamic Page Cache now stores one variation per account for authenticated users. The Internal Page Cache is different — it keys entries on the URL alone and ignores cache contexts entirely, so it keeps serving one variant to every anonymous user no matter what contexts bubbled up. That is a correctness risk, not a bypass; the only way to vary anonymous output is to disable that module. What keeps a response out of Dynamic Page Cache is a leaf with `'max-age' => 0`, which also makes the whole render tree uncacheable; the Internal Page Cache is different again, because it ignores max-age entirely — `PageCache::storeResponse()` stores entries as `Cache::PERMANENT` and expires them by tag — so only the `page_cache_kill_switch` service or a `no_cache: TRUE` route keeps a page out of that one. The remedy is not to strip metadata (that produces wrong output) but to move the volatile piece behind a `#lazy_builder` placeholder so the bubble stops at that boundary.",
    },
    {
