...
`,
  ...
  interview: [
    ...
    },
    {
      ...
      a: "That `max-age: 0` bubbles all the way to the root of the render tree, so it does not just make that block uncacheable — it makes the entire page `UNCACHEABLE (poor cacheability)` for Dynamic Page Cache and unstoreable for Internal Page Cache and any reverse proxy. One block just cost the site every caching layer it had. The correct tool is cache tags: give the block the tags of whatever it depends on and invalidate them when that data changes, so it is cached until it is genuinely wrong. If the data really is per-request — a countdown, a random pick — put it behind a `#lazy_builder` so the `max-age: 0` stays inside the placeholder and never reaches the page.",
    },
    {
